...
export default function Relatorio(){
    ...
    const formReport = z.object({
        ...
        materia: z.string().nonempty('Selecionar uma Opção'),
    ...
    return(
        <section className={styles.container}>
            <form id='formRelatorio' onSubmit={handleSubmit} className={styles.form}>
                ...
                <div className={styles.containerInput}>
                    ...
                    <div className={styles.containerInputDate}>
                        <label className={styles.labelSelectDate}>
                            ...
                            <input type="date"
                            onFocus={()=>setInputError({... inputError, beginningPeriod:{error: false, onFocus: true, textError: ''}})}
                            
                            value={inputReport.beginningPeriod.toISOString().substring(0, 10)} onChange={(el : React.FormEvent<HTMLInputElement>)=>{
                                if(el.currentTarget.value === ''){
                                    setInputReport({... inputReport, beginningPeriod: new Date()})
                                }else{
                                    setInputReport({... inputReport, beginningPeriod: new Date(el.currentTarget.value)})
                                }
                            }}
                            style={addStyle(inputError.beginningPeriod.error)}/>

                            {inputError.beginningPeriod.error && <InputError text={inputError.beginningPeriod.textError} style={true} />}
                        </label>

                        <label className={styles.labelSelectDate}>
                            <span>Período de Término:</span>
                            <input type="date"
                            onFocus={()=>setInputError({... inputError, endPeriod:{error: false, onFocus: true, textError: ''}})}
                            
                            value={inputReport.endPeriod.toISOString().substring(0, 10)} onChange={(el : React.FormEvent<HTMLInputElement>)=>{
                                if(el.currentTarget.value === ''){
                                    setInputReport({... inputReport, endPeriod: new Date()})
                                }else{
                                    setInputReport({... inputReport, endPeriod: new Date(el.currentTarget.value)})
                                }
                            }} 
                            style={addStyle(inputError.endPeriod.error)}/>

                            {inputError.endPeriod.error && <InputError text={inputError.endPeriod.textError} style={true} />}
                        </label>
                    </div>
                    {inputError.endPeriod.error && <InputError text={inputError.endPeriod.textError} style={true} />}
                </div>
                <div className={styles.containerInput}>
                    ...
                    <label className={styles.labelSelect}>
                        <select 
                        onFocus={()=>setInputError({... inputError, orderBy:{error: false, onFocus: true, textError: ''}})}

                        value={inputReport.orderBy} onChange={(el : React.FormEvent<HTMLSelectElement>)=>{
                            setInputReport({... inputReport, orderBy: el.currentTarget.value})
                        }} 
                        style={addStyle(inputError.orderBy.error)}>
                            ...
                            <option value={orderBy.DistributionDate}>Data de Início</option>
                            <option value={orderBy.ConclusionDate}>Data de Conclusão</option>
                            <option value={orderBy.Category}>Categoria</option>
                        </select>
                    </label>
                    {inputError.orderBy.error && <InputError text={inputError.orderBy.textError} style={false} />}
                </div>
                ...
                <div className={styles.containerFilter}>
                        ...
                        <div className={styles.containerInputFilter}>
                            ...
                            <label className={styles.labelSelect}>
                                ...
                                value={inputReport.userId} onChange={(el : React.FormEvent<HTMLSelectElement>)=>{
                                    setInputReport({... inputReport, userId: Number(el.currentTarget.value)})
                                }} 
                            ...
                            {inputError.userId.error && <InputError text={inputError.userId.textError} style={false} />}
                        </div>
                        ...
                        <div className={styles.containerInputFilter}>
                            ...
                            <label className={styles.labelSelect}>
                                ...
                                value={inputReport.categoryId} onChange={(el : React.FormEvent<HTMLSelectElement>)=>{
                                    setInputReport({... inputReport, categoryId: Number(el.currentTarget.value)})
                                }} 
                        ...
                        {inputError.categoryId.error && <InputError text={inputError.categoryId.textError} style={false} />}
                        </div>
                        ...
                        <div className={styles.containerInputFilter}>
                            ...
                            <label className={styles.labelSelect}>
                                ...
                                value={inputReport.name} onChange={(el : React.FormEvent<HTMLSelectElement>)=>{
                                    setInputReport({... inputReport, name: el.currentTarget.value})
                                }} 
                        ...
                        {inputError.name.error && <InputError text={inputError.name.textError} style={false} />}
                        </div>
                        ...
                        <div className={styles.containerInputFilter}>
                            ...
                            <label className={styles.labelSelect}>
                                ...
                                value={inputReport.materia} onChange={(el : React.FormEvent<HTMLSelectElement>)=>{
                                    setInputReport({... inputReport, materia: el.currentTarget.value})
                                }} 
                        ...
                        {inputError.materia.error && <InputError text={inputError.materia.textError} style={false} />}
                        </div>
                        ...
                        <div className={styles.containerInputFilter}>
                                ...
                                <label className={styles.labelSelect}>
                                    <select 
                                    onFocus={()=>setInputError({... inputError, status:{error: false, onFocus: true, textError: ''}})}

                                    value={inputReport.status} onChange={(el : React.FormEvent<HTMLSelectElement>)=>{
                                        setInputReport({... inputReport, status: el.currentTarget.value})
                                    }} 
                                    style={addStyle(inputError.status.error)}>
                                        <option value=''>Selecione um Status</option>
                                        <option value='None'>Sem Status</option>
                                        <option value={Status.NaoVisualizado}>{Status.NaoVisualizado}</option>
                                        <option value={Status.EmAguardo}>{Status.EmAguardo}</option>
                                        <option value={Status.AguardandoRetorno}>{Status.AguardandoRetorno}</option>
                                        <option value={Status.EmInicializacao}>{Status.EmInicializacao}</option>
                                        <option value={Status.EmAndamento}>{Status.EmAndamento}</option>
                                        <option value={Status.Concluido}>{Status.Concluido}</option>
                                        
                                    </select>
                                </label>
                            {inputError.status.error && <InputError text={inputError.status.textError} style={false} />}
                        </div>
                </div>

            </form>

                <input className={styles.labelInputBtn} type='submit' form='formRelatorio' value='Baixar Arquivo' />
